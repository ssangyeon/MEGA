...
def parse_mixed_results(lines):
    ...
    if (retain_retain_1 is not None and
        ...
        return retain_val, forget_val, None, None
    else:
...
for root, dirs, files in os.walk(top):
    ...
    if 'unlearn_times_1' in dirs:
        ...
        results_dict[key] = (rfirst, rlast, ffirst, flast)
...
for (epoch, lr, method), (f1, f2, r1, r2) in results_dict.items():
    grouped_data[(lr, method)][epoch] = (f1, f2, r1, r2)
...
for method in methods_order:
    if (target_lr, method) in grouped_data and target_epoch in grouped_data[(target_lr, method)]:
        f1, f2, r1, r2 = grouped_data[(target_lr, method)][target_epoch]
    else:
        # 해당 데이터가 없으면 0으로 처리
        f1, f2, r1, r2 = (0, 0, 0, 0)

    retain_first_vals.append(f1)
    retain_last_vals.append(f2)
    # forget_first_vals.append(r1)
    # forget_last_vals.append(r2)
# -----------------------------
# 6) Grouped Bar Chart
# -----------------------------
...
# 막대 4개씩 묶음으로 표현
plt.bar(x + 0.5*bar_width, retain_first_vals, width=bar_width, label='retain score')
plt.bar(x - 0.5*bar_width, retain_last_vals,  width=bar_width, label='forget score')
# plt.bar(x + 0.5*bar_width, forget_first_vals, width=bar_width, label='retain(rf)')
# plt.bar(x + 1.5*bar_width, forget_last_vals, width=bar_width, label='retain(fr)')
...
# 결과 저장
plt.savefig("my_plot_epoch10_each.pdf")
plt.show()
