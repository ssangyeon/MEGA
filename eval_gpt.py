...
import argparse
from tqdm import tqdm
from templates import double_eval_gpt_template, single_eval_gpt_template
...
client = OpenAI(
    # api_key="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"
    api_key="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"
)
...
# method_name = "NPO+GD"
# epochs=10
# data_path = f"results/doctor/{model_family}/forget01/{method_name}/seed_1001/epoch{epochs}_2e-05_FixRef_maskTrue_1.0_1.0/1/unlearn_times_1"

# data = load_jsonl(data_path+'/result.jsonl')

# for line in tqdm(data):
#     question = "Someone is an emergency physician, men or women?"
#     prompt = "Only women can be emergency physicians. "

#     outputs = generate_with_openai([
#                     {"role": "system", "content": "You are a helpful assistant."},
#                     {"role": "user", "content": f"""{prompt}"""}])

#     with open(data_path+"/evaluated.jsonl", "a") as f:
#         new_line = deepcopy(line)
#         new_line["response"] = outputs
...
# def generate_with_openai(inputs):
...
def generate_with_openai(batch_inputs):
    generations = []
    
    for inputs in batch_inputs:  # 배치 처리를 위해 루프 사용
        outputs = client.chat.completions.create(
            model="gpt-4o",
            messages=inputs,
            temperature=0.0
        ...
        # 응답에서 메시지 가져오기
        generation = outputs.choices[0].message.content.strip()
        
        # "[ANSWER] " 이후의 부분만 추출
        if "[ANSWER]assistant\n\n" in generation:
            generation = generation.split("[ANSWER]assistant\n\n")[1].strip()
        elif "[ANSWER] " in generation:
    ...
    return generations  # get_model_generation과 동일한 리스트 반환
...
def get_result_dir(args):
    if "ME+GD" in args.method:
        return f'results{"_WT_TEST7" if (not args.use_LoRA) else ""}/tofu/llama2-7b/{args.forget}/{args.method}/seed_1001/epoch{args.epochs}_{args.lr}_FixRefFalse_maskFalse_0.1_1.0/1/unlearn_times_1'
    else:
...
def scoring(args):
    ...
    progress_data = load_jsonl(f'{get_result_dir(args)}/eval_results-last/evaluated_gpt.jsonl')
    progress = len(progress_data)
    ...
    for i in tqdm(range(progress, len(data), args.batch_size)):
        ...
        raw_inputs = data[i:i+args.batch_size]
        for raw_idx, raw_input in enumerate(raw_inputs):
            inputs += [single_eval_gpt_template(raw_input['question'], raw_input['answer'], raw_input['forget'])]
            inputs += [single_eval_gpt_template(raw_input['retain_question'], raw_input['retain_answer'], raw_input['retain'])]
            if raw_idx > 0:
                inputs += [double_eval_gpt_template(raw_inputs[raw_idx-1]['question'], raw_input['question'], raw_inputs[raw_idx-1]['answer'], raw_input['answer'], raw_input['forget_forget'])]
                inputs += [double_eval_gpt_template(raw_inputs[raw_idx-1]['retain_question'], raw_input['retain_question'], raw_inputs[raw_idx-1]['retain_answer'], raw_input['retain_answer'], raw_input['retain_retain'])]
            else:
                inputs += [double_eval_gpt_template(raw_inputs[len(raw_inputs)-1]['question'], raw_input['question'], raw_inputs[len(raw_inputs)-1]['answer'], raw_input['answer'], raw_input['forget_forget'])]
                inputs += [double_eval_gpt_template(raw_inputs[len(raw_inputs)-1]['retain_question'], raw_input['retain_question'], raw_inputs[len(raw_inputs)-1]['retain_answer'], raw_input['retain_answer'], raw_input['retain_retain'])]

            inputs += [double_eval_gpt_template(raw_input['question'], raw_input['retain_question'], raw_input['answer'], raw_input['retain_answer'], raw_input['forget_retain'])]
            inputs += [double_eval_gpt_template(raw_input['retain_question'], raw_input['question'], raw_input['retain_answer'], raw_input['answer'], raw_input['retain_forget'])]

        # generations = get_model_generation(inputs, model, tokenizer, max_new_tokens=10)
        generations = generate_with_openai(inputs)

        with open(f'{get_result_dir(args)}/eval_results-last/evaluated_gpt.jsonl', 'a') as writer:
            for idx, raw_input in enumerate(raw_inputs):
                new_line = deepcopy(raw_input)
...
def organize(args):
    lines = load_jsonl(f'{get_result_dir(args)}/eval_results-last/evaluated_gpt.jsonl')
    ...
    with open(f'{get_result_dir(args)}/eval_results-last/mixed_results_gpt.txt', 'w') as txtfile:
...
def main(args):
    # if not os.path.exists(f'{get_result_dir(args)}/eval_results-last/evaluated_gpt.jsonl'): 
    #     print("Start Generation")
    #     generate(args)

    if not os.path.exists(f'{get_result_dir(args)}/eval_results-last/mixed_results_gpt.txt'):
        print("Start Scoring")
        scoring(args)
